...
async function fetchSalaryData() {
  ...
  if (USE_GOOGLE_SHEETS_DATA) {
    try {
      ...
      console.error(
        "Failed to fetch from Google Sheets, falling back to hardcoded data:",
        error
      );
...
function processGoogleSheetsData(rawData) {
  ...
  const validData = rawData.filter(
    (item) => item.company && item.hourlySalary > 0
  );
  ...
  const companyMappings = {
    ...
    "Confidential - Small Company": [
      "Confidential - Small Company",
      "חסוי - חברה קטנה",
      "חברת סייבר בינלאומית קטנה",
    ],
    "Defense Industry": ["Defense Industry", "תעשיה ביטחונית"],
    "Intl Bank (Tech Division)": [
      "Intl Bank (Tech Division)",
      "בנק בינלאומי במחלקת טכנולוגיה",
    ],
  ...
  validData.forEach((item) => {
    ...
    const canonicalCompany =
      aliasToCanonical[originalCompany.toLowerCase()] || originalCompany;
  ...
  Object.keys(companyData).forEach((company) => {
    ...
    const avgSalary =
      Math.round(
        (salaries.reduce((sum, sal) => sum + sal, 0) / salaries.length) * 100
      ) / 100;
    ...
    if (americanCompanies.includes(company)) {
      ...
      if (
        americanIndicators.some((indicator) => companyLower.includes(indicator))
      ) {
        category = "american";
      } else if (
        israeliIndicators.some((indicator) => companyLower.includes(indicator))
      ) {
        category = "israeli";
      } else {
      }
  ...
  const newCompanies = Object.keys(companyData).filter(
    (company) => !knownCompanies.includes(company)
  );
  if (newCompanies.length > 0) {
    console.log(
      "🆕 New companies detected (consider adding to mapping):",
      newCompanies
    );
  }
...
function getSalariesForCompanies(companies, salaryType) {
  ...
  return companies.map(
    (company) => dataSource.salaries[salaryType][company] || 0
  );
}
...
async function initializeSalaryCharts() {
  try {
    ...
    document.getElementById(
      "total-average"
    ).textContent = `שכר ממוצע של סטודנט למדעי המחשב:  ₪${totalAvg} לשעה`;
    document.getElementById(
      "american-average"
    ).textContent = `ממוצע כללי: ₪${americanAvg} לשעה`;
    document.getElementById(
      "israeli-average"
    ).textContent = `ממוצע כללי: ₪${israeliAvg} לשעה`;
    document.getElementById(
      "other-average"
...
function populateDataTable(salaryData) {
  ...
  const categories = [
    {
      name: "american",
      label: "חברות אמריקאיות",
      companies: salaryData.companies.american,
    },
    {
      name: "israeli",
      label: "חברות ישראליות",
      companies: salaryData.companies.israeli,
    },
...
function categorizeCompany(companyName) {
  ...
  if (
    americanIndicators.some((indicator) =>
      lowerName.includes(indicator.toLowerCase())
    )
  ) {
    return "american";
  } else if (
    israeliIndicators.some((indicator) =>
      lowerName.includes(indicator.toLowerCase())
    )
  ) {
    return "israeli";
...
function renderTable() {
  ...
  const avgSalary =
    filteredTableData.length > 0
      ? Math.round(
          filteredTableData.reduce((sum, item) => sum + item.salary, 0) /
  ...
  resultsCounter.innerHTML = `
        ...
        ${
          filteredEntries > 0
            ? `| ממוצע מסונן: <strong>₪${avgSalary}</strong>`
            : ""
        }
    `;
    ...
    filteredTableData = allTableData.filter(
      (item) =>
        ...
        (item.previousExperience &&
          typeof item.previousExperience === "string" &&
          item.previousExperience.toLowerCase().includes(term)) ||
        (item.pensionFund &&
          typeof item.pensionFund === "string" &&
          item.pensionFund.toLowerCase().includes(term))
    );
  }
  renderTable();
}
